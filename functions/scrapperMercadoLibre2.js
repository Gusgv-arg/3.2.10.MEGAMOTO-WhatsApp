...
export const scrapperMercadoLibre2 = async () => {
	...
	for (const url of urls) {
		...
		console.log("Resultados disponibles--->", resultadosDisponibles);
		...
		const products = $(
			...
			.map((i, el) => {
				const titulo =
					$(el).find("h2.poly-component__title > a").text() ||
				...
				const link =
					$(el).find("h2.poly-component__title > a").attr("href") ||
					"Link no disponible";
				const ubicacion =
					$(el).find(".poly-component__location").text() ||
					"Ubicación no disponible";
				const vendedor =
					$(el).find(".poly-component__seller").text() ||
	...
	if (allProducts.length > 0) {
		...
		console.log(
			"Datos exportados a productos.xlsx y return del array allProducts"
		);
		return allProducts;
	} else {
		console.warn(
